...
class DriverScreen extends Component {
    ...
    componentDidMount() {
        ...
        messaging().onMessage(async remoteMessage => {
            ...
            if (remoteMessage.data.title == "Seal Number Information") {
                ...
                let isExist = R.findIndex(R.propEq("humanTaskID", infoSeal.ticket_no))(this.state.rawData)
                if (isExist >= 0) {
                    let rawData = Object.assign([], this.state.rawData)
                    rawData[isExist] = { ...rawData[isExist], doc_info: { ...rawData[isExist].doc_info, "delivery": { ...rawData[isExist].doc_info.delivery, "no": infoSeal.delivery_order, "seal_no": infoSeal.seal_no } } }
                    this.setState({ rawData })
                }
            }
            ...
            if (remoteMessage.data.title == "Gate Security Check In") {
                ...
                let isExist = R.findIndex(R.propEq("humanTaskID", infoGate.ticket_no))(this.state.rawData)
                if (isExist >= 0) {
                    let rawData = Object.assign([], this.state.rawData)
                    rawData[isExist] = { ...rawData[isExist], doc_info: { ...rawData[isExist].doc_info, "delivery": { ...rawData[isExist].doc_info.delivery, "leg_org_ETD": infoGate.start_time, "leg_dest_ETA": "NONE", "security_name_out": "NONE", "security_plant_out": "NONE", "security_id_out": "NONE", "security_name_in": infoGate.security_name, "security_id_in": infoGate.security_id, "security_plant_in": infoGate.security_plant } } }
                    this.setState({ rawData })
                }
            }
            ...
            if (remoteMessage.data.title == "Gate Security Check Out") {
                ...
                if (isExist >= 0) {
                    let rawData = Object.assign([], this.state.rawData)
                    rawData[isExist] = { ...rawData[isExist], doc_info: { ...rawData[isExist].doc_info, "delivery": { ...rawData[isExist].doc_info.delivery, "leg_dest_ETA": infoGate.stop_time, "security_name_out": infoGate.security_name, "security_id_out": infoGate.security_id, "security_plant_out": infoGate.security_plant } } }
                    this.setState({ rawData })
                }
            }
        })
    ...
    getDataTaskList = async (limit, offset, tabs) => {
        ...
        let payload = {
            ...
            offset: offset,
            params: {
                humanTaskType: "DRIVER_ASSIGNMENT",
        ...
        let res = ""
        let response = await Api.create().getCountHumanTaskListByAssignmentID(payload)
                ...
                <ParalaxLayout
                    ...
                    card={(rowData, data, rowID, collapse, index) => (
                        <CardTicket
                            ...
                            onPress={() => this.props.navigation.navigate('TicketDetailScreen', { data, rowData, allData: this.state.allData })} />
                    )}
                    onChangeTabs={(tabs) => {
