...
class CardBasePrice extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            data: null,
    ...
    getDataMaterial = async (basepriceID) => {
        let materialData = []
        const payload = { limit: 100, offset: 0, params: { kimap: "", basepriceID } }
        const res = await Api.create().getMaterialBasePriceItem(payload)
        if (res.data && res.data.data) {
            res.data.data && res.data.data.map((value) => {
                if (value) materialData.push(value)
                return null
            })
    ...
    handleSign = async () => {
        ...
        let res = await Api.create().postSignatureBasePrice(payload)
    ...
    handleVerify = async (userID) => {
        ...
        let res = await Api.create().verifySignatureBasePrice(payload)
...
export default connect(mapStateToProps, null)(CardBasePrice)