...
class CardReplenishment extends React.Component {
    ...
    getData = async (source) => {
        ...
        if (response.status == "S") {
            this.getDataMaterial(response.data.mvMaterialInventory)
    ...
    getDataMaterial = async (inventoryID) => {
        let materialData = []
        const payload = { limit: 100, offset: 0, params: { kimap: "", inventoryID: inventoryID ? inventoryID.objectID : "" } }
    ...
    handleSign = async () => {
        ...
        let res = await Api.create().postSignatureValuation(payload)
    ...
    handleVerify = async (userID) => {
        ...
        let res = await Api.create().verifySignatureValuation(payload)
    ...
    render() {
        ...
        return (
            <View>
                <CardOrder
                    date={data ? M(data.mvDocDate, "DD-MM-YYYY").format("DD MMMM YYYY") : "-"}
                ...
                {this.borderTop()}
                <CardNote title="Notes" desc={data ? data.mvNote : "-"} />
