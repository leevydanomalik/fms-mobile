...
class CardReplenishmentPack extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            data: null,
    ...
    handleSign = async () => {
        ...
        let res = await Api.create().postSignatureReplenishmentPacking(payload)
    ...
    handleVerify = async (userID) => {
        ...
        let res = await Api.create().verifySignatureReplenishmentPacking(payload)
    ...
    render() {
        ...
        let from = data && data.pckReplenishmentPicking && data.pckReplenishmentPicking.pcReplensihmentOrder && data.pckReplenishmentPicking.pcReplensihmentOrder.mrFrom && data.pckReplenishmentPicking.pcReplensihmentOrder.mrFrom.cesStorageBinQuantID && data.pckReplenishmentPicking.pcReplensihmentOrder.mrFrom.cesStorageBinQuantID.binQuantLocCode
        let fromDesc = data && data.pckReplenishmentPicking && data.pckReplenishmentPicking.pcReplensihmentOrder && data.pckReplenishmentPicking.pcReplensihmentOrder.mrFrom && data.pckReplenishmentPicking.pcReplensihmentOrder.mrFrom.cesStorageBinQuantID && data.pckReplenishmentPicking.pcReplensihmentOrder.mrFrom.cesStorageBinQuantID.binQuantName
        let to = data && data.pckReplenishmentPicking && data.pckReplenishmentPicking.pcReplensihmentOrder && data.pckReplenishmentPicking.pcReplensihmentOrder.mrTo && data.pckReplenishmentPicking.pcReplensihmentOrder.mrTo.cesStorageBinQuantID && data.pckReplenishmentPicking.pcReplensihmentOrder.mrTo.cesStorageBinQuantID.binQuantLocCode
        let toDesc = data && data.pckReplenishmentPicking && data.pckReplenishmentPicking.pcReplensihmentOrder && data.pckReplenishmentPicking.pcReplensihmentOrder.mrTo && data.pckReplenishmentPicking.pcReplensihmentOrder.mrTo.cesStorageBinQuantID && data.pckReplenishmentPicking.pcReplensihmentOrder.mrTo.cesStorageBinQuantID.binQuantName
        const delivery = [
            {
            ...
            // {
            //     title: data && data.pckReplenishmentPicking && data.pckReplenishmentPicking.pcReplensihmentOrder && data.pckReplenishmentPicking.pcReplensihmentOrder.mrFrom && data.pckReplenishmentPicking.pcReplensihmentOrder.mrFrom.cesStorageBinQuantID && data.pckReplenishmentPicking.pcReplensihmentOrder.mrFrom.cesStorageBinQuantID.binQuantLocCode,
            //     to: "From",
            //     desc: data && data.pckReplenishmentPicking && data.pckReplenishmentPicking.pcReplensihmentOrder && data.pckReplenishmentPicking.pcReplensihmentOrder.mrFrom && data.pckReplenishmentPicking.pcReplensihmentOrder.mrFrom.cesStorageBinQuantID && data.pckReplenishmentPicking.pcReplensihmentOrder.mrFrom.cesStorageBinQuantID.binQuantName,
            // },
            // {
        ...
        const materialData = []
        data && data.pckReplenishmentPicking ? data.pckReplenishmentPicking.pcMaterials && data.pckReplenishmentPicking.pcMaterials.map((value) => {
            ...
            <View>
                ...
                <View style={{ backgroundColor: "#fff", padding: 15, paddingBottom: 20 }}>
                    <CardContract
                        ...
                        service={'PDT'}
                        date={data ? M(data.pckPlanDate, "DD-MM-YYYY").format("DD MMMM YYYY") : "-"} />
                </View>

                <ComponentSpace />
                <CardMaterial title="Materials" data={data && materialData} />
                {this.borderTop()}
                <CardNote title="Notes" desc={data ? data.pckNote : "-"} />
...
export default connect(mapStateToProps, null)(CardReplenishmentPack)