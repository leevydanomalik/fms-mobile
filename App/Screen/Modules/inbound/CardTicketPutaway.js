...
class CardTicketPutaway extends Component {
    constructor(props) {
        super(props)
        this.state = {
            visible: false,
    ...
    componentDidMount() {
        ...
        console.log('data', data.item.payload.pcDesc)
    }
    ...
    render() {
        ...
        let poNumber = datas && datas.ptGoodReceipt && datas.ptGoodReceipt.grInboundDelivery && datas.ptGoodReceipt.grInboundDelivery.idPurchaseOrder && datas.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.objectID
        let docDate = datas ? datas.ptDocDate : ""
        let pickers = this.props.auth.user.data.userName
        let description = datas.ptDesc ? datas.ptDesc : "This vehicle must stop the engine while parking and please securing surrounding and confirm to loading master"
        if (poNumber) {
            shipto = {
                name: datas && datas.ptGoodReceipt.grInboundDelivery.idPurchaseOrder && datas.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poShipTo && datas.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poShipTo.cesPlantID && datas.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poShipTo.cesPlantID.plantName,
                desc: datas && datas.ptGoodReceipt.grInboundDelivery.idPurchaseOrder && datas.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poShipTo && datas.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poShipTo.cesPlantID && datas.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poShipTo.cesPlantID.plantDesc
            }
            billto = {
                name: datas && datas.ptGoodReceipt.grInboundDelivery.idPurchaseOrder && datas.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poBillTo && datas.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poBillTo.cesPlantID && datas.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poBillTo.cesPlantID.plantName,
                desc: datas && datas.ptGoodReceipt.grInboundDelivery.idPurchaseOrder && datas.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poBillTo && datas.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poBillTo.cesPlantID && datas.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poBillTo.cesPlantID.plantDesc
            }
        }
        return (
                    ...
                    {!collapse ? (
                        ...
                        <View>
                            ...
                            <View style={{ marginTop: 20, flexDirection: "row", alignItems: "center", alignContent: "center", justifyContent: "space-between" }}>
                                <Text style={{ color: "#000", fontSize: 18, fontWeight: "bold" }}>{`Putaway ${poNumber}`}</Text>
...
export default connect(mapStateToProps, null)(CardTicketPutaway)