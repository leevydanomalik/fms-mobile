...
class CardInbound extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            data: null,
    ...
    render() {
        ...
        const delivery = [
            {
                title: data && data.idPurchaseOrder.objectID && data.idPurchaseOrder.poShipTo && data.idPurchaseOrder.poShipTo.cesPlantID.plantName,
                to: "Ship To",
                desc: data && data.idPurchaseOrder.objectID && `${data.idPurchaseOrder.poShipTo.cesPlantID.plantAddress.streetName} ${data.idPurchaseOrder.poShipTo.cesPlantID.plantAddress.kelurahanID.subZipcode} ${data.idPurchaseOrder.poShipTo.cesPlantID.plantAddress.provinceID.data.dtName} ${data.idPurchaseOrder.poShipTo.cesPlantID.plantAddress.countryID.data.dtName}`
            },
            {
                title: data && data.idPurchaseOrder.objectID && data.idPurchaseOrder.poBillTo && data.idPurchaseOrder.poBillTo.cesPlantID.plantName,
                to: "Bill To",
                desc: data && data.idPurchaseOrder.objectID && `${data.idPurchaseOrder.poBillTo.cesPlantID.plantAddress.streetName} ${data.idPurchaseOrder.poBillTo.cesPlantID.plantAddress.kelurahanID.subZipcode} ${data.idPurchaseOrder.poBillTo.cesPlantID.plantAddress.provinceID.data.dtName} ${data.idPurchaseOrder.poBillTo.cesPlantID.plantAddress.countryID.data.dtName}`
            }
        ]

        const contract = [
            { 
                title: data && data.idPurchaseOrder.objectID && data.idPurchaseOrder.poSupplierPlant && data.idPurchaseOrder.poSupplierPlant.cesPlantID.plantName, 
                subtitle: `${data && data.idPurchaseOrder.objectID && data.idPurchaseOrder.poSupplierPlant && data.idPurchaseOrder.poSupplierPlant.cesPlantID.plantDesc}` 
            },
            { 
                title: data && data.idPurchaseOrder.objectID && data.idPurchaseOrder.poShipTo && data.idPurchaseOrder.poShipTo.cesPlantID.plantName, 
                subtitle: `${data && data.idPurchaseOrder.objectID && data.idPurchaseOrder.poShipTo && data.idPurchaseOrder.poShipTo.cesPlantID.plantDesc}` 
            }
        ]

        const materialData = data ? data.idPurchaseOrder.poMaterials : []
        ...
        return (
            <View>
                <CardOrder
                    date={data && M(data.idDocDate, "DD-MM-YYYY").format("DD MMMM YYYY")}
                ...
                <CardDocument
                    ...
                    fromTo={{ title: contract[0].title, date: data ? M(data.idPlanDate, "DD-MM-YYYY").format("DD MMMM YYYY") : "-" }}
                    shipTo={{ title: contract[1].title, date: data ? M(data.idPlanDate, "DD-MM-YYYY").format("DD MMMM YYYY") : "-" }}
                />
                ...
                <View style={{ backgroundColor: "#fff", padding: 15, paddingBottom: 20 }}>
                    <CardContract
                        data={data ? contract : []}
                        service={'PDT'}
                        date={data ? M(data.idPlanDate, "DD-MM-YYYY").format("DD MMMM YYYY") : "-"} />
                </View>
...
export default connect(mapStateToProps, null)(CardInbound)