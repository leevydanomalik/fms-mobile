...
class CardTaskStoring extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            materials: []
    ...
    setData = () => {
        ...
        let msMaterialStorings = datas && datas.msMaterialStorings ? datas.msMaterialStorings : []
        msMaterialStorings && msMaterialStorings.map((value) => {
            return materials.push({
                ...value,
                fromBQs: value.msBinQuant.msBinQuant,
                quantityPick: value.msQty
            })
        })
    ...
    render() {
        ...
        let odID = datas && datas.msPutaway && datas.msPutaway.ptGoodReceipt && datas.msPutaway.ptGoodReceipt.grInboundDelivery ? datas.msPutaway.ptGoodReceipt.grInboundDelivery.objectID : ""
        let poNumber = datas && datas.msPutaway && datas.msPutaway.ptGoodReceipt && datas.msPutaway.ptGoodReceipt.grInboundDelivery && datas.msPutaway.ptGoodReceipt.grInboundDelivery.idPurchaseOrder && datas.msPutaway.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.objectID
        let docDate = datas ? datas.msDocDate : ""
        let pickers = this.props.auth.user ? this.props.auth.user.data.userName.toUpperCase() : ""
        let description = datas ? datas.msDesc : ""
        if (poNumber) {
            shipto = {
                name: datas && datas.msPutaway && datas.msPutaway.ptGoodReceipt.grInboundDelivery.idPurchaseOrder && datas.msPutaway.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poShipTo && datas.msPutaway.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poShipTo.cesPlantID && datas.msPutaway.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poShipTo.cesPlantID.plantName,
                desc: datas && datas.msPutaway && datas.msPutaway.ptGoodReceipt.grInboundDelivery.idPurchaseOrder && datas.msPutaway.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poShipTo && datas.msPutaway.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poShipTo.cesPlantID && datas.msPutaway.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poShipTo.cesPlantID.plantDesc
            }
            billto = {
                name: datas && datas.msPutaway && datas.msPutaway.ptGoodReceipt.grInboundDelivery.idPurchaseOrder && datas.msPutaway.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poBillTo && datas.msPutaway.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poBillTo.cesPlantID && datas.msPutaway.ptGoodReceipt.grInboundDelivery.idPurchaseOrder.poBillTo.cesPlantID.plantName,
            ...
            <View>
                    ...
                    <View style={{marginTop: 0, backgroundColor: "#fff", padding: 20}}>
                        ...
                        <View style={{flexDirection: "row"}}>
                            ...
                            <View style={{flex: 1, marginLeft: 10, justifyContent: 'center',}}>
                                ...
                                <Text style={{fontSize: 10}}>Storing #{data.item.sourceID}</Text>
                ...
                {(
                    <View style={{marginTop: 10, backgroundColor: "#fff", padding: 20}}>
                        ...
                        <View style={{flex: 1}}>
                            <Text
                                style={{color: "#000", fontSize: 16, fontWeight: "bold"}}>{`Storing ID#${odID}`}</Text>
                ...
                {(
                    <View style={{marginTop: 10, backgroundColor: "#fff", padding: 20}}>
                        <View style={{
                            ...
                            <Text style={{fontSize: 16, fontWeight: "bold", color: "#999"}}>Storing List</Text>
                        </View>

                        {materials.map((item, index) => {
                            const {msMaterial, msHUID, quantityPick, fromBQs} = item
                            return (
                                <View key={index}
                                    ...
                                    <View style={{
                                        ...
                                        <View style={{flex: 1, marginLeft: 10, alignContent: "space-between"}}>
                                            <View style={{flex: 1}}>
                                                <Text style={{
                                                    ...
                                                    fontSize: 12,
                                                    fontWeight: "bold"
                                                }}>{msMaterial.material.materialKimap}</Text>
                                                <Text style={{
                                                    color: "#fff",
                                                    fontSize: 10
                                                }}>{msMaterial.material.materialName}</Text>
                                            ...
                                            <Text style={{
                                                ...
                                                fontSize: 16,
                                                fontWeight: "bold"
                                            }}>{quantityPick} {msMaterial.material.materialUoM && msMaterial.material.materialUoM.value}</Text>
                                        </View>
                                        <TouchableOpacity onPress={onVerifiedPress}>
                                            <View style={{
                                                ...
                                                <QRCode size={70} value={msMaterial.material.materialKimap}/>
                                            </View>
                                        </TouchableOpacity>
                                    </View>
                                    ...
                                    <View style={{flexDirection: "row", justifyContent: "space-between"}}>
                                        <TouchableOpacity onPress={onVerifiedPress}
                                            ...
                                            <View style={{
                                                ...
                                                <QRCode size={90} value={msMaterial.material.materialKimap}/>
                                            </View>
                                        </TouchableOpacity>
                                        <TouchableOpacity onPress={onVerifiedPress}
                                                          style={{alignItems: "center", alignContent: "center"}}>
                                            <Text
                                                style={{color: "#000", fontSize: 12, marginBottom: 5}}>{msHUID}</Text>
                                            <View style={{
                                                borderWidth: 3,
                                                borderColor: Colors.error,
                                                width: 100,
                        ...
                        {/* <View style={{ paddingTop: 15 }}>
                            <View style={{ flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 15 }}>
                                <Text style={{ fontSize: 16, fontWeight: "bold", color: "#000" }}>SL#9686968886</Text>
...
export default connect(mapStateToProps, null)(CardTaskStoring)