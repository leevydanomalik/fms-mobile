...
class CardPIR extends React.Component {
    ...
    handleSign = async () => {
        ...
        let res = await Api.create().postSignaturePir(payload)
    ...
    handleVerify = async (userID) => {
        ...
        let res = await Api.create().verifySignaturePir(payload)
    ...
    render() {
        ...
        const delivery = [
            {
                title: data && data.dsp && data.dsp.cesPlantID.plantName,
                to: "Ship To",
                desc: data && data.dsp && `${data.dsp.cesPlantID.plantAddress.streetName} ${data.dsp.cesPlantID.plantAddress.kelurahanID.subZipcode} ${data.dsp.cesPlantID.plantAddress.provinceID.data.dtName} ${data.dsp.cesPlantID.plantAddress.countryID.data.dtName}`
            },
            {
        ...
        console.log('pirDocDate', data)

        return (
            <View>
                <CardOrder
                    date={data && M(data.pirDocDate, "DD-MM-YYYY").format("DD MMMM YYYY")}
                    photo={"https://st2.depositphotos.com/1011434/7519/i/950/depositphotos_75196567-stock-photo-handsome-man-smiling.jpg"}
                    role={"Purchase Info Record Manager"}
                    movement={"Material Movement #303"}
                    warehouse={data && data.es.plant && data.es.plant.plantName} />

                <ComponentSpace />
                <CardDocument
                    type={"Purchase Info Record ID"}
                    docID={data ? data.objectID : ""}
                    desc={data && data.pirDesc ? data.pirDesc : "-"}
                    pointer={{ title: routeMaterialName, step: "Single Step (45m20s)" }} //"80 Box Mesran 80 UPC001 / 8 Pallet"
                    fromTo={{ title: 'NONE', date: 'NONE' }}
                    shipTo={{ title: data && data.es.plant && data.es.plant.plantName, date: data ? M(data.pirPlanDate, "DD-MM-YYYY").format("DD MMMM YYYY") : "-" }}
                />
                ...
                <View style={{ backgroundColor: "#fff", padding: 15, paddingBottom: 20 }}>
                    <CardContract
                        ...
                        service={'PDT'}
                        date={data ? M(data.pirPlanDate, "DD-MM-YYYY").format("DD MMMM YYYY") : "-"} />
                </View>

                <ComponentSpace />
                <CardMaterial title="Materials" data={data && materialData} />
                {this.borderTop()}
                <CardNote title="Notes" desc={data ? data.pirNote : "-"} />
