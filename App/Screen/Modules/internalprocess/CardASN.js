...
class CardASN extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            data: null,
    ...
    handleSign = async () => {
        ...
        let res = await Api.create().postSignatureAsn(payload)
    ...
    handleVerify = async (userID) => {
        ...
        let res = await Api.create().verifySignatureAsn(payload)
    ...
    render() {
        ...
        console.log('kelurahan', data && data.purchaseOrder.poShipTo.cesPlantID.plantAddress)
        const delivery = [
            {
                title: data && data.asnTo && data.asnTo.cesPlantID.plantName,
                to: "Ship To",
                desc: data && data.asnTo && `${data.asnTo.cesPlantID.plantAddress.streetName} ${data.asnTo.cesPlantID.plantAddress.kelurahanID.subZipcode} ${data.asnTo.cesPlantID.plantAddress.provinceID.data.dtName} ${data.asnTo.cesPlantID.plantAddress.countryID.data.dtName}`
            },
            {
                title: data && data.purchaseOrder && data.purchaseOrder && data.purchaseOrder.poBillTo.cesPlantID.plantName,
                to: "Bill To",
                desc: data && data.purchaseOrder && data.purchaseOrder && data.purchaseOrder.poBillTo && `${data.purchaseOrder.poBillTo.cesPlantID.plantAddress.streetName} ${data.purchaseOrder.poBillTo.cesPlantID.plantAddress.kelurahanID.subZipcode} ${data.purchaseOrder.poBillTo.cesPlantID.plantAddress.provinceID.data.dtName} ${data.purchaseOrder.poBillTo.cesPlantID.plantAddress.countryID.data.dtName}`
            }
        ]
        const materialData = []
        ...
        return (
            <View>
                <CardOrder
                    date={data && M(data.docDate, "DD-MM-YYYY").format("DD MMMM YYYY")}
                ...
                <CardDocument
                    ...
                    fromTo={{ 
                        title: data && data.asnFrom && data.asnFrom.cesPlantID.plantName, 
                        date: data ? M(data.deliveryDate, "DD-MM-YYYY").format("DD MMMM YYYY") : "-" 
                    }}
                    shipTo={{ 
                        title: data && data.asnTo && data.asnTo.cesPlantID.plantName, 
                        date: data ? M(data.deliveryDate, "DD-MM-YYYY").format("DD MMMM YYYY") : "-" 
                ...
                <View style={{ backgroundColor: "#fff", padding: 15, paddingBottom: 20 }}>
                    <CardContract
                        data={[
                            { 
                                title: data && data.asnFrom && data.asnFrom.cesPlantID.plantName, 
                                subtitle: data && data.asnFrom && data.asnFrom.cesPlantID.plantDesc
                            },
                            { 
                                title: data && data.asnTo && data.asnTo.cesPlantID.plantName, 
                                subtitle: data && data.asnTo && data.asnTo.cesPlantID.plantDesc
                            }
                        ]}
                        service={'PDT'}
                        date={data ? M(data.deliveryDate, "DD-MM-YYYY").format("DD MMMM YYYY") : "-"} />
                ...
                {this.borderTop()}
                <CardNote title="Notes" desc={data ? data.note : "-"} />
...
export default connect(mapStateToProps, null)(CardASN)