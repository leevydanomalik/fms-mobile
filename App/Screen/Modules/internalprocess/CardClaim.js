...
class CardClaim extends React.Component {
    ...
    handleSign = async () => {
        ...
        let res = await Api.create().postSignatureClaim(payload)
    ...
    handleVerify = async (userID) => {
        ...
        let res = await Api.create().verifySignatureClaim(payload)
    ...
    render() {
        ...
        const contract = [
            { 
                title: data && data.claimPurchaseOrder && data.claimPurchaseOrder.poSupplierPlant.cesPlantID.plantName, 
                subtitle: data && data.claimPurchaseOrder && data.claimPurchaseOrder.poSupplierPlant.cesPlantID.plantDesc 
            },
            { 
                title: data && data.shipto && data.shipto.cesPlantID.plantName, 
                subtitle: data && data.shipto && data.shipto.cesPlantID.plantDesc 
            }
        ]
        
        const materialData = []

        data && data.claimMaterials.map((items) => {
            materialData.push({
                qtyUoM: items.qty,
                ...items
            })
        ...
        return (
            <View>
                <CardOrder
                    date={data && M(data.claimDocDate, "DD-MM-YYYY").format("DD MMMM YYYY")}
                ...
                <CardDocument
                    ...
                    desc={data && data.claimDesc ? data.claimDesc : "-"}
                    pointer={{ title: routeMaterialName, step: "Single Step (45m20s)" }}
                    fromTo={{ title: contract[0].title, date: data && M(data.claimPlanDate, "DD-MM-YYYY").format("DD MMMM YYYY") }}
                    shipTo={{ title: contract[1].title, date: data && M(data.claimPlanDate, "DD-MM-YYYY").format("DD MMMM YYYY") }}
                />
                ...
                <View style={{ backgroundColor: "#fff", padding: 15, paddingBottom: 20 }}>
                    <CardContract
                        data={contract}
                        service={'PDT'}
                        date={data ? M(data.claimPlanDate, "DD-MM-YYYY").format("DD MMMM YYYY") : "-"} />
                </View>

                <ComponentSpace />
                <CardMaterial title="Materials" data={data && materialData} />
                {this.borderTop()}
                <CardNote title="Notes" desc={data ? data.claimNote : "-"} />
...
export default connect(mapStateToProps, null)(CardClaim)