...
class CardShipping extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            data: null,
    ...
    handleSign = async () => {
        ...
        let res = await Api.create().postSignatureShipping(payload)
    ...
    handleVerify = async (userID) => {
        ...
        let res = await Api.create().verifySignatureShipping(payload)
    ...
    render() {
        ...
        const materialData = data ? data.shMaterial : []

        if (data) {
            delivery = [
                {
                    ...delivery[0],
                    title: data && data.shPurchaseOrder && data.shPurchaseOrder.poShipTo.cesPlantID.plantName,
                    desc: data && data.shPurchaseOrder && `${data.shPurchaseOrder.poShipTo.cesPlantID.plantAddress.streetName} ${data.shPurchaseOrder.poShipTo.cesPlantID.plantAddress.kelurahanID.subZipcode} ${data.shPurchaseOrder.poShipTo.cesPlantID.plantAddress.provinceID.data.dtName} ${data.shPurchaseOrder.poShipTo.cesPlantID.plantAddress.countryID.data.dtName}`
                },
                {
                    ...delivery[1],
                    title: data && data.shPurchaseOrder && data.shPurchaseOrder.poBillTo.cesPlantID.plantName,
                    desc: data && data.shPurchaseOrder && `${data.shPurchaseOrder.poBillTo.cesPlantID.plantAddress.streetName} ${data.shPurchaseOrder.poBillTo.cesPlantID.plantAddress.kelurahanID.subZipcode} ${data.shPurchaseOrder.poBillTo.cesPlantID.plantAddress.provinceID.data.dtName} ${data.shPurchaseOrder.poShipTo.cesPlantID.plantAddress.countryID.data.dtName}`
                }
            ]
        }
        ...
        if (data) {
            contract = [
                { 
                    ...contract[0],
                    title: data && data.shPurchaseOrder && data.shPurchaseOrder.poSupplierPlant.cesPlantID.plantName, 
                    subtitle: data && data.shPurchaseOrder && data.shPurchaseOrder.poSupplierPlant.cesPlantID.plantDesc 
                },
                { 
                    ...contract[1],
                    title: data && data.shPurchaseOrder && data.shPurchaseOrder.poShipTo.cesPlantID.plantName, 
                    subtitle: data && data.shPurchaseOrder && data.shPurchaseOrder.poShipTo.cesPlantID.plantDesc 
                }
            ]
        }
        ...
        return (
            <View>
                <CardOrder
                    date={data && M(data.shDocDate, "DD-MM-YYYY").format("DD MMMM YYYY")}
                ...
                <CardDocument
                    ...
                    desc={data && data.shDesc ? data.shDesc : "-"}
                    pointer={{ title: routeMaterialName, step: "Single Step (45m20s)" }}
                    fromTo={{ title: contract[0].title, date: data ? M(data.shPlanDate, "DD-MM-YYYY").format("DD MMMM YYYY") : "-" }}
                    shipTo={{ title: contract[1].title, date: data ? M(data.shPlanDate, "DD-MM-YYYY").format("DD MMMM YYYY") : "-" }}
                />
                ...
                <View style={{ backgroundColor: "#fff", padding: 15, paddingBottom: 20 }}>
                    <CardContract
                        data={data && contract}
                        service={'PDT'}
                        date={data ? M(data.shPlanDate, "DD-MM-YYYY").format("DD MMMM YYYY") : "-"} />
                </View>

                <ComponentSpace />
                <CardMaterial title="Materials" data={data && materialData} />
                {this.borderTop()}
                <CardNote title="Notes" desc={data ? data.shNote : "-"} />
...
export default connect(mapStateToProps, null)(CardShipping)